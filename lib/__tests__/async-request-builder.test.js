'use strict';
const http_module = require('../async-modules/http');
...
describe('async-request-builder', () => {
    ...
    test('creates rejected promise if no async method', (done) => {
    ...
    test('creates rejected promise if async module does not exist', (done) => {
        asyncRequestLayer = asyncRequestLib(requestDefFile, {});
    ...
    test('uses mockResponse if defined', (done) => {
    ...
    test('closeAsyncModuleConnections waits for pending requests before closing connections', (done) => {
    ...
    test('waitForPendingRequests waits request to complete', (done) => {
        let asyncHelper = setupPendingAsyncRequests();
    ...
    test('waits for pending requests before closing connections', (done) => {
        let asyncHelper = setupPendingAsyncRequests();
        let closeResultPromise = asyncRequestLayer.closeAsyncModuleConnections();
        // expect(asyncHelper.asyncRequestCompleted).toBe(false);
        closeResultPromise.then(() => {
        ...
        asyncHelper = setupPendingAsyncRequests();
        closeResultPromise = asyncRequestLayer.closeAsyncModuleConnections();
        // expect(asyncHelper.asyncRequestCompleted).toBe(false);
        closeResultPromise.then(() => {
            expect(asyncHelper.asyncRequestCompleted).toBe(true);
            done();
