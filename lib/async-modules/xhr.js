...
const getAsyncFn = (operationDetails, parentOperationDetails, globals) => {
    ...
    if (VALID_METHODS.indexOf(operationDetails.method) === -1) {
        const pathReplaceRequestHandler = params => {
            replaceParams(methodUrl, params, false);
        };
        return pathReplaceRequestHandler;
    } else {
        const xhrAsyncRequestHandler = requestObject => {
            sendXHRPromise(createFinalRequestObject(methodUrl, urlPrefix, operationDetails.method,
                operationDetails.headers, requestObject));
        };
        return xhrAsyncRequestHandler;
    }
};
