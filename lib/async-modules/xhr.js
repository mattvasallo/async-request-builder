...
const sendXHRPromise = (url, method, headers, payload) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            console.info("Unable to make xmlHttpRequest.");
            reject(err);
        }
    });
};
...
const sendHttpRequest = ({
    ...
    if (typeof(fetch) === "function") {
        promiseResult = fetch(url, {
            ...
            err => {
                //should only get here due to network disconnected
                console.info("Unable to make fetch request.");
                return Promise.reject(err);
            });
    } else if (typeof(XMLHttpRequest) === "function") {
        promiseResult = sendXHRPromise(url, method, headers, payload);
    } else {
        promiseResult = Promise.reject("Fetch and XmlHttpRequest not available!");
    }
...
const getAsyncFn = (operationDetails, parentOperationDetails, globals) => {
    ...
    if (operationDetails.method === "REPLACE") {
        //simply replace params in url
        return requestObject => Promise.resolve(replaceParams(methodUrl, requestObject.params));
    } else if (VALID_METHODS.indexOf(operationDetails.method) === -1) {
        return () => Promise.reject(`Invalid XHR method ${operationDetails.method}`);
    }

    return requestObject => sendHttpRequest(createFinalRequestObject(methodUrl, urlPrefix, operationDetails.method,
