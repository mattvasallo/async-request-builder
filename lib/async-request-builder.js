...
const loadAsyncModule = (moduleName, loadedAsyncModuleMap) => {
    const isBuiltInModule = moduleName[0] !== '.' && moduleName[0] !== '/';
    const asyncModulePath = isBuiltInModule ? ASYNC_MODULES_FOLDER + moduleName : moduleName;
    // if (!loadedAsyncModuleMap[asyncModulePath]) {
    // try {
    //     loadedAsyncModuleMap[asyncModulePath] = require(asyncModulePath);
    // } catch (e) {
    //     console.info("Failed to load module type", moduleName);
    // }
    // }
...
module.exports = function createAsyncRequestBuilder(requestJSON, config) {
    const globals = {
        ...
        "loadedAsyncModuleMap": {
            "./async-modules/xhr": require("./async-modules/xhr"),
            "./async-modules/mysql": require("./async-modules/mysql"),
            "./async-modules/mongo": require("./async-modules/mongo"),
            "./async-modules/http": require("./async-modules/http"),
            "./async-modules/file": require("./async-modules/file"),
            "./async-modules/aggregator": require("./async-modules/aggregator")
        }
    };
